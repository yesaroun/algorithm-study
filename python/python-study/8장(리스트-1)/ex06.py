...
# insert() 메서드 : 리스트명.insert(인덱스, 추가할 요소) 원하는 인덱스에 추가할 수 있다.
li1 = ['tv', '오디오', 'pc', '키보드']
...
# 2번째 : 리스트명.remove("요소"), 리스트에서 해당하는 요소가 있으면 삭제한 후, 요소를 반환하지 않는다.
# 3번째 : del 리스트명[인덱스], 리스트에서 인덱스에 해당하는 요소를 삭제한 후, 요소를 반환하지 않는다.
hero = ["스파이더맨", '배트맨', '슈퍼맨', '아이언맨', '헐크', '홍당무']
element = hero.pop(0)
...
# 리스트의 모든 요소를 삭제를 하고자 한다면, 리스트명.clear()를 사용하면 된다.
hero = ["스파이더맨", '배트맨', '슈퍼맨', '아이언맨', '헐크', '태권v', '배트맨']
element = hero.clear()
print(element)
print(hero)
