...
print("--------------------")
# 비어 있는 세트를 만드는 방법 {}를 사용하지 아니하고 set() 호출하면 된다.
set4 = set()
# 세트에 값을 추가하기(add() 사용)
...
# 세트는 요소가 해싱이 가능하려면 해쉬코드를 가져야 한다고 하였다.
# 그 값이 변경되면 안된다. 따라서 세트는 변경가능한 항목을 가지면 안된다.
# 하여, 아래와 같이 리스트를 요소로 가지지 못하고 에러를 발생시킨다.
# set5 = {1, 2.2, "car", [10, 20]}
# 튜플은 값 변경이 이루어지지 않는 객체이므로 세트의 요소가 될 수 있다.
set5 = {1, 2.2, "car", (10, 20)}
print(set5)

# 여러 개의 요소를 추가하려 update() 메서드를 사용하면 된다.
set6 = set()
# 아래 update() 문자열 "인간"을 문자로 나누어서 세트에 저장이 된다.
set6.update("인간", [1, 43, -10])
print(set6)
set61 = set()
# 아래 update() 문다열 "인간"을 문자로 나누지 아니하고 세트에 저장이 된다.
set61.update(["인간", 1, 3, -10])
print(set61)
print("--------------------")
# 세트의 요소를 삭제를 하고자 할 때 discard() 리턴값이 없다.
set6.discard(1)
print(set6)
