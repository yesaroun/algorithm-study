...
# 아래 코드는 이미 앞서서 배운바 있다. 내장함수를 이용하여 최소값, 최대값을 구하는 코드이다.
num = [ 1, 5, -9, 100 ]
print("최소값 : ", min(num))
print("최대값 : ", max(num))
...
for i in range(1, len(prices)):
    ...
    # 조건절에서 부등호를 수정을 하면 최대값을 구할 수 있다.
...
shortedst_word = words[0]
list_word = []
for i in range(1, len(words)):
    if len(words[i]) <= len(shortedst_word):
        if len(words[i]) < len(shortedst_word):
            list_word.clear()
            list_word.append(words[i])
        else:
            list_word.append(shortedst_word)
            shortedst_word = words[i]
            list_word.append(shortedst_word)
