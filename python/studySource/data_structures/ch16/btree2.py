...
class BTree:
    ...
    def search_bfs(self, elem):
        ...
        while not queue_.is_empty() and queue_.peek().elem != elem:
            root = queue_.peek()
            queue_.dequeue()

            if root.left_child is not None:
                queue_.enqueue(root.left_child)

            if root.right_child is not None:
                queue_.enqueue(root.right_child)

        ret = None if queue_.is_empty() else queue_.peek()
        return ret
