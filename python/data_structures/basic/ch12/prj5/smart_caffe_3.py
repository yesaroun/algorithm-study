...
while True:
    ...
    if choice == 3:
        ...
        pay = eval(input("결제를 위한 현금 입력: "))

        while True:
            if pay < choiced_price:
                print("금액이 {}원 부족합니다.".format(choiced_price - pay))
                extra_pay = eval(input("추가 금액 입력: "))
                pay += extra_pay
            else:
                break

        print("지불 총금액: {}원, 거스름돈: {}원".format(pay, pay - choiced_price))
        choiced_price = 0
